...
int good(int r,int c,int n,int m){
    if(r>=0 && r<n && c>=0 && c<m) return true;
    return false;
}
void dfs(int r,int c,vector<vector<char>> &a,vector<vector<int>>&vis){
    int n=a.size(),m=a[0].size();
    for(int j=0;j<8;j++){
    	// pr(r+dx[j],c+dy[j]);
    	// pr("NO RE");
        if(good(r+dx[j],c+dy[j],n,m) && vis[r+dx[j]][c+dy[j]]==0 && a[r+dx[j]][c+dy[j]]=='#'){
            vis[r+dx[j]][c+dy[j]]=1;
            // pr("Gone");
            dfs(r+dx[j],c+dy[j],a,vis);
        }
    }
}
void solve()
