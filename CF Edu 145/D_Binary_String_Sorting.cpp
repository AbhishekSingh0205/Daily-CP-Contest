...
void solve()
{
    ...
    es(s);int n=s.size();
    int pref[n][2];int suff[n][2];memset(pref,0,sizeof pref);memset(suff,0,sizeof suff);
    fl(i,0,n){
        if(s[i]=='1'){
            pref[i][1]++;
        }
    ...
    for(int i=n-1;i>=0;i--){
        ...
        if(i!=n-1){
            suff[i][0]+=suff[i+1][0];suff[i][1]+=suff[i+1][1];
        }
    }
    int ans=9223372036854775807; 
    int val=1000000000000;
    fl(i,0,n){
        if(s[i]=='0'){
            // ith index=0
            int tmp=pref[i][1];
            if(i+1<n) tmp+=suff[i][0];
            ans=min(ans,tmp*val+tmp);
        }
        else if(s[i]=='1'){
            int tmp=0;
            if(i-1>=0){
            ...
            if(i+1<n && s[i+1]=='0'){
                int tmpAns=val;
                int tmp=0;
                if(i-1>=0) tmp=pref[i-1][1];
                if(i+2<n) tmp+=suff[i+2][0];
                ans=min(ans,tmp*val + tmp + tmpAns);
            }
        }
        // ith index 1
    }    
