...
void solve()
{
    ...
    int sum=0;
    for(int j=31;j>=0;j--){
        int prefE[n][2];memset(prefE,0,sizeof prefE);
        int prefO[n][2];memset(prefO,0,sizeof prefO);
        ...
        fl(i,0,n){
            /*
                Logic : 
                    Arr = [                       ]
                    Prefix Sum bit ka
                    Length ka prefix sum of even and odd.
                    Count of even legth and odd length
            */
           if(i){
                pref[i]=pref[i-1];
                prefE[i][0]=prefE[i-1][0];
                prefE[i][1]=prefE[i-1][1];
                prefO[i][0]=prefO[i-1][0];
                prefO[i][1]=prefO[i-1][1];
           }
           if(a[i] & (1LL<<j)) pref[i]++;
           if(pref[i]%2==0){
                prefE[i][0]+=i;
        ...
        for(int i=0;i<n;i++){
            ...
            if(a[i]&(1LL<<j)){
                if(pref[i]%2==0){
                    int tmp = prefE[n-1][0],cnt = prefE[n-1][1];
                    if(i){
                        tmp -= prefE[i-1][0];
                        cnt -= prefE[i-1][1];
                    }
                    ...
                    int tmp = prefO[n-1][0],cnt = prefO[n-1][1];
                    if(i){
                        tmp -= prefO[i-1][0];
                        cnt -= prefO[i-1][1];
                    }
