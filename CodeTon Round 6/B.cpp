...
int fun(int input1,int input2[],int input3){
    int n = input1;
    ...
    while(q.size()){
        ...
        lvl[tp.first] = tp.second;
        m[tp.second].push_back(tp.first);
        q.pop();
        if(tp.first*2+1<=n){
            q.push({tp.first*2+1,tp.second+1});
        }
        if(tp.second*2+2<=n){
            q.push({tp.first*2+2,tp.second+1});
        }
    }
    int lv= lvl[input3];
    vector<int> res;
    for(auto v:m[lv]){
        if(v!=input3) res.push_back(v);
    }
    return res;
